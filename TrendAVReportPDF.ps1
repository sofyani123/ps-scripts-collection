...
$destinationBasePath = "S:\Business Change and Infrastructure\IT\Reports\SDDC TrendDS TrendAV Reports"
...
# Get all PDF files that match the patterns
$pdfFiles = @()
$pdfFiles += Get-ChildItem -Path $directoryPath -Filter "Summary_Report*.pdf"
$pdfFiles += Get-ChildItem -Path $directoryPath -Filter "Computer_Report*.pdf"
$pdfFiles += Get-ChildItem -Path $directoryPath -Filter "Anti-Malware_Report*.pdf"
...
foreach ($pdfFile in $pdfFiles) {
    ...
    if ($extractedDate) {
        ...
        $baseName = [System.IO.Path]::GetFileNameWithoutExtension($pdfPath) -replace " .*", ""
        $newFileName = "${baseName} ${extractedDate}.pdf"
